/**
 * Test script — Monthly CPR scan with threshold 0.3%
 * Run: node src/cron/jobs/testMonthlyCPR.js
 */
...
const THRESHOLD  = 1; // test threshold
...
async function testMonthlyCPR() {
  const cprFor = currentMonthLabel();
  console.log(`[TEST-MonthlyCPR] Running scan... (CPR for: ${cprFor}, threshold: ${THRESHOLD}%)`);
  ...
  const filepath = path.join(OUTPUT_DIR, `${scanDate}_monthly_test.json`);
  fs.writeFileSync(filepath, JSON.stringify(result, null, 2), 'utf8');

  console.log(`[TEST-MonthlyCPR] ${narrow.length} stocks found, saved: ${filepath}`);
  console.log(JSON.stringify(result, null, 2));
}

testMonthlyCPR().catch(console.error);
